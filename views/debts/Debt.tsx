...
import { addItem, delItem, editItem, getData, setData, borderBillionMillionThousand, replaceSpace } from '../tools/iosys';
...
export default memo(function Debt() {
  ...
  const accs = useRef(emptyAccount().accounts);
  ...
  const onStart = async () => {
    ...
    accs.current = await getAccounts();
    await getItems(await getAccounts());
    data.debts.map(item => {
      if (item.date != '') {
        const [year, month, day] = item.date.split('-').map(Number);
        const date = new Date(year, month - 1, day);
        item.date = `${date.getDate()} ${monthNames[date.getMonth()]} ${date.getFullYear()}`;
      }
    });
    setState(data);
  };
  ...
  const onClick = async () => {
    ...
    if (editing.current.editing) {
      dat.id = state.debts[editing.current.index].id;
      dat.id_account = Number(pickerValue);
      newDat.debts[editing.current.index] = dat;
      await editItem('debts', 'Debt', editing.current.index, dat);
    } else {
  ...
  const getAcc = (id_account: string | number) => {
    try {
      return accs.current.filter(account => {
  ...
  const openEditModal = async (index: number) => {
    const data: debt = await getData({
      fileName: 'Debt'
    });
    setSelectedDate(state.debts[index].date);
    setText([state.debts[index].name, `${state.debts[index].sum}`, state.debts[index].contact, state.debts[index].comment, data.debts[index].date]);
  ...
  return <View style={{
    ...
    height: '100%'
            ...
            <ModalWindow visible={visible} setVisible={setVisible} buttonTextLeft='Должны мне' buttonTextRight='Должен я' activeModalButton={activeModalButton} setActiveModalButton={setActiveModalButton} colorActiveLeft='#3FDEAE' colorActiveRight='#3FDEAE' functionCancelButton={() => {
      setText(['', '', '', '']);
      setSelectedContact('');
      setSelectedDate('');
      setPickerValue('');
    }} functionSaveButton={tryToSave}>
                <Input textName='Название' value={text[0].toString()} setItems={setText} index={0} placeholder='Введите название долга' keyboardType='default' colorActiveInput='#3FDEAE' />
                <Input textName='Сумма' value={text[1].toString()} setItems={setText} index={1} placeholder='Введите сумму долга' keyboardType='numeric' colorActiveInput='#3FDEAE' />
                <InputDate functionDate={useCallback(() => {
        setDatePickerVisible(true);
      }, [setDatePickerVisible])} textName='Крайняя дата' value={selectedDate.toString()} setValue={useCallback(() => {
        setSelectedDate;
      }, [setSelectedDate])} placeholder='Введите дату' keyboardType='default' colorActiveInput='##3FDEAE' />
                <InputContact textName='Контакт' functionConctact={selectContact} value={selectedContact} setValue={setSelectedContact} placeholder='Введите контакт для связи (не обязательно)' keyboardType='phone-pad' colorActiveInput='#3FDEAE' />
                <Input textName='Комментарий' value={text[3].toString()} setItems={setText} index={3} placeholder='Введите комментарий к долгу (не обязательно)' keyboardType='default' colorActiveInput='#3FDEAE' />
                <PickerBlock>
                    <TextName>Счёт</TextName>
                    <DropDownPicker open={openPicker} value={pickerValue} translation={{
          PLACEHOLDER: 'Выберите счёт!',
          NOTHING_TO_SHOW: 'Нет счетов для выбора!'
        }} setOpen={setOpenPicker} setValue={setPickerValue} items={items} setItems={setItems} containerStyle={{
          width: '66%',
          alignSelf: 'flex-end'
        ...
        setDatePickerVisible(false);
            ...
            <Modal animationType='fade' transparent={true} visible={winInfo} onRequestClose={() => {
      setIdCard(-1);
      setWinInfo(false);
    }}>
                <View style={{
        ...
        backgroundColor: 'rgba(0, 0, 0, 0.3)'
            ...
            <Header name='Долги' style='1' functionLeft={useCallback(() => {}, [])} functionRight={useCallback(() => {
      if (items.length > 0) {
        setVisible(true);
        setActiveModalButton(debtTome);
      } else {
        Alert.alert('Внимание!', 'Вы пробуете создать долг, не создав предварительно счёт!');
      }
    }, [setVisible, setActiveModalButton, items, debtTome])} onModalHide={useCallback(async () => {
      onStart();
    }, [])} />
            <ButtonTypeView>
                <ButtonType onPress={() => {
        ...
        borderColor: debtTome ? '#3FDEAE' : '#C9C9C9'
      }}>
                    <ButtonTypeText style={{
        ...
        borderColor: !debtTome ? '#3FDEAE' : '#C9C9C9'
          ...
          fontFamily: 'MainFont-Regular'
            ...
            <Scroll>
                {state.debts && state.debts.map((item, index) => {
        {
          if (debtTome && item.type == '1' || item.type == '2' && !debtTome) return <CardSwipe key={index} onDelete={useCallback(() => {
            del(index);
          }, [])} onEdit={useCallback(() => {
            openEditModal(index);
