...
import { Text, View, StyleSheet, FlatList, useWindowDimensions, Image, Animated, ScrollView, Button, Modal, TouchableOpacity, Alert, TouchableWithoutFeedback, Keyboard } from 'react-native';
...
import { account, category, emptyCategories, emptyHistory, history, emptyAccount } from '../../models/interfaces';
...
import { getData, setData, addItem, delItem, editItem, borderBillionMillionThousand, replaceSpace } from '../tools/iosys';
...
function calculateValues(categories: category, history: history): category {
  ...
  return {
    categories: [...categories.categories.map(categ => {
      let sum = 0;
...
export default memo(function Home() {
  ...
  const activeModalButton = useRef(true);
  ...
  const editingcategory = useRef({
    editingcategory: false,
    index: 0
  });
  const debtTome = useRef(true);
  ...
  const [itemsAccounts, setitemsAccounts] = useState<{
    label: string;
    value: string;
  }[]>([]);
  ...
  const [touchItemIndex, setTouchItemIndex] = useState({
    move: 0,
    index: -1
  });
  ...
  const colors = ['#FF0000', '#FFA500', '#FFFF00', '#008000', '#0000FF', '#800080', '#FFC0CB', '#A52A2A', '#808080', '#3FDEAE', '#9966cc'];
  ...
  const editModalHistory = async (index: number) => {
    settexthistory([datahistory.history[index].sum.toString(), datahistory.history[index].comment, datahistory.history[index].date]);
    setPickerValue(`${datahistory.history[index].category}`);
    setPickerValueAccounts(`${datahistory.history[index].id_account}`);
    setVisibleAddHistory(debtTome.current);
  ...
  function getItems(accounts: category['categories']) {
    ...
    accounts.map(item => {
      data.push({
        label: item.name + ', ' + item.category_type,
  ...
  const onStart = async () => {
    let dataC: category = await getData({
      fileName: 'category'
    });
    let dataH: history = await getData({
      fileName: 'history'
    });
  ...
  const handleAddCategory = async () => {
    ...
    if (editingcategory.current.editingcategory) {
      dat.id = mama.categories[editingcategory.current.index].id;
      mama.categories[editingcategory.current.index] = dat;
      editItem('categories', 'category', editingcategory.current.index, dat);
      setDataItems(calculateValues(JSON.parse(JSON.stringify(mama)), JSON.parse(JSON.stringify(dataH))));
    } else {
  ...
  const handleADDHistory = async () => {
    ...
    if (mama.history.length !== 0) {
      ...
      if (res === 'not-found') Alert.alert('ошибка', 'счет не найден');else {
        if (res === 'no-money') Alert.alert('ошибка', 'недостаточно средств');else {
          if (editing.editing) {
    ...
    height: '100%'
        ...
        justifyContent: 'flex-start'
                    ...
                    <View style={styles.modalContainer}>
                        {colors.map((color, index) => <TouchableOpacity key={color} style={[styles.colorBlock, {
            backgroundColor: color
          }, selectedBlockIndex === index && {
            borderColor: 'black',
            borderWidth: 3
          }]} onPress={() => handleColorSelect(color, index)} onPressOut={() => setSelectedBlockIndex(-1)} />)}
                    </View>
            ...
            <ModalWindow functionCancelButton={() => {
      setText(['', '', '', '']);
      editingcategory.current = {
        editingcategory: false,
        index: -1
      ...
      handleAddCategory();
    }} visible={VisibleAddCategory} setVisible={setVisibleAddCategory} buttonTextLeft='Доход' buttonTextRight='Расход' activeModalButton={activeModalButtonAddCategory} setActiveModalButton={setActiveModalButtonAddCategory} colorActiveLeft='#3EA2FF' colorActiveRight='#FF6E6E'>
                <PickerBlock>
                    <Text style={{
          fontSize: 15,
          textAlign: 'center',
          ...
          marginRight: '37%'
                        ...
                        <Button title='выбери цвет' onPress={() => setVisible2(true)} color={selectedColor} />
                    </View>
                </PickerBlock>
            ...
            <ModalWindowHistory visible={visibleHistory} setVisible={setVisibleHistory} type={false}>
                <Container>
                    {datahistory.history && datahistory.history.map((item, index) => {
          {
            const category = dataItems.categories.find(cat => cat.id === item.category);
            return <CardWithButtons key={index} func={useCallback(() => {
              setTouchItemIndex({
                move: 2,
                index: index
              });
              setVisible3(true);
            }, [setTouchItemIndex, setVisible3])}>
                                        <View style={{
                width: '100%'
              }}>
                                            <View style={{
                  ...
                  flexDirection: 'row'
                    ...
                    flexDirection: 'row'
                                                    ...
                                                    <Circle radius={10} color={category && category.color ? category.color : '#000000'} />
                      ...
                      fontFamily: 'MainFont-Regular'
                                                        ...
                                                        {category && category.name ? category.name.length > 12 ? `${category.name.slice(0, 12)}...` : category.name : 'No category name available'}
                                                    </Text>
                                                </View>
                                                <Text style={{
                    color: '#8D97A2',
            ...
            <ModalWindowOneButton functionCancelButton={() => {
      settexthistory(['', '', '', '']), setPickerValue(''), setSelectedDate(''), setPickerValueAccounts('');
      setEditing({
        editing: false,
        index: -1
      });
    }} functionSaveButton={handleADDHistory} visible={visibleAddHistory} setVisible={setVisibleAddHistory} windowName={editing.editing ? 'Редактирование операции' : 'Создание операции'} colorActive={activeModalButtonHistory ? '#3EA2FF' : '#FF6E6E'}>
                <InputDate functionDate={useCallback(() => {
        setDatePickerVisible(true);
      }, [setDatePickerVisible])} textName='Дата' value={selectedDate.toString()} setValue={useCallback(() => {
        setSelectedDate;
      }, [setSelectedDate])} placeholder='Введите дату' keyboardType='default' colorActiveInput={activeModalButtonHistory ? '#3EA2FF' : '#FF6E6E'} />
                <PickerBlock>
                    <TextName>Категория</TextName>

                    <DropDownPicker open={openPicker} value={pickerValue} setOpen={setOpenPicker} setValue={setPickerValue} items={items} setItems={setItems} containerStyle={{
          width: '66%',
          alignSelf: 'flex-end',
          position: 'relative'
        }} zIndex={2} placeholder='Выберите категорию' dropDownDirection='BOTTOM' />
                </PickerBlock>

                <Input textName='Сумма' value={texthistory[0].toString()} setItems={settexthistory} index={0} placeholder='Введите сумму ' keyboardType='numeric' colorActiveInput={activeModalButtonHistory ? '#3EA2FF' : '#FF6E6E'} />
                <Input textName='Комментарий' value={texthistory[1].toString()} setItems={settexthistory} index={1} placeholder='Введите комментарий' keyboardType='default' colorActiveInput={activeModalButtonHistory ? '#3EA2FF' : '#FF6E6E'} />
                <PickerBlock>
                    <TextName>Счет</TextName>
                    <DropDownPicker open={openPickerAccounts} value={pickerValueAccounts} setOpen={setOpenPickerAccounts} setValue={setPickerValueAccounts} items={itemsAccounts} setItems={setitemsAccounts} containerStyle={{
          width: '66%',
          alignSelf: 'flex-end'
        }} placeholder='Выберите счет' dropDownDirection='BOTTOM' zIndex={1} />
                </PickerBlock>
                {isDatePickerVisible && <DateTimePicker style={{
        flex: 1,
        position: 'relative'
      }} isVisible={isDatePickerVisible} mode='date' onConfirm={(date: Date) => {
        ...
        height: '100%'
                    ...
                    <HeaderViewModal>
                        <BackArrowSvg width={25} height={25} onPress={() => {
            setVisible(false);
          }} />
          ...
          justifyContent: 'space-around'
            ...
            borderColor: categorytype ? '#3EA2FF' : '#C9C9C9'
          }}>
                            <Text style={{
              fontFamily: 'MainFont-Regular',
            ...
            borderColor: !categorytype ? '#FF6E6E' : '#C9C9C9'
          }}>
                            <Text style={{
              fontFamily: 'MainFont-Regular',
              ...
              if (categorytype && item.category_type == 'Доход' || item.category_type == 'Расход' && !categorytype) return <CardWithButtons key={index} func={useCallback(() => {
                setTouchItemIndex({
                  move: 1,
                  index: index
                });
            ...
            <Modal animationType='fade' transparent={true} visible={visible3} onRequestClose={() => {
      setVisible3(false);
      setTouchItemIndex({
        move: 0,
        index: -1
        ...
        backgroundColor: 'rgba(0, 0, 0, 0.3)'
            ...
            top: 13
                        ...
                        {touchItemIndex.move == 1 ? <AlertTextContainer>
                                <AlertInView style={{
              justifyContent: 'flex-start'
            }}>
                                    <AlertMessage style={{
                ...
                height: '70%',
                width: '30%',
                backgroundColor: touchItemIndex.index !== -1 ? `${dataItems.categories[touchItemIndex.index].color}` : '#fff'
              }}></View>
                                </AlertInView>
                                <AlertInView>
                                    <AlertMessage style={{
                ...
                textAlign: 'center'
                                        ...
                                        {touchItemIndex.index !== -1 ? dataItems.categories[touchItemIndex.index].name : ''}
                                    </AlertMessage>
                                </AlertInView>
                            </AlertTextContainer> : <AlertTextContainer>
                                <AlertInView>
                                    <AlertMessage style={{
                width: '50%',
                textDecorationLine: 'underline'
                ...
                textAlign: 'center'
                                        ...
                                        {touchItemIndex.index !== -1 ? dataItems.categories[dataItems.categories.findIndex(item => item.id === datahistory.history[touchItemIndex.index].category)].name : ''}
                                    </AlertMessage>
                                </AlertInView>
                                <AlertInView>
                                    <AlertMessage style={{
                ...
                textAlign: 'center'
                                        ...
                                        {touchItemIndex.index !== -1 ? PeopleDate(datahistory.history[touchItemIndex.index].date) : ''}
                                    </AlertMessage>
                                </AlertInView>
                                <AlertInView>
                                    <AlertMessage style={{
                ...
                textAlign: 'center'
                                        ...
                                        {touchItemIndex.index !== -1 ? `${borderBillionMillionThousand(datahistory.history[touchItemIndex.index].sum)} руб` : ''}
                                    </AlertMessage>
                                </AlertInView>
                                <AlertInView>
                                    <AlertMessage style={{
                ...
                textAlign: 'center'
                                        ...
                                        {touchItemIndex.index !== -1 ? datahistory.history[touchItemIndex.index].comment : ''}
                                    </AlertMessage>
                                </AlertInView>
                                <AlertInView>
                                    <AlertMessage style={{
                ...
                textAlign: 'center'
                                        ...
                                        {touchItemIndex.index !== -1 ? `${itemsAccounts[itemsAccounts.findIndex(item => +item.value === datahistory.history[touchItemIndex.index].id_account)].label} руб` : ''}
                                    </AlertMessage>
                                </AlertInView>
                            </AlertTextContainer>}
            ...
            <Header name='Категории' style='2' functionLeft={useCallback(() => {}, [])} functionRight={useCallback(async () => {
      await getAccount(await getAccounts());
      setVisibleHistory(true);
    }, [setVisibleHistory])} onModalHide={useCallback(async () => {
      onStart();
    }, [])} />
            <View style={{
      height: '100%',
      ...
      width: '100%'
            ...
            opacity: animateContainerOpacity
                            ...
                            <DonutChart data={dataItems.categories.filter(item => {
              return item.value > 0;
            })} size={300} />
                        </Animated.View>
                        <View style={styles.ViewInDiagrams}>
                            ...
                            <Text style={styles.TextInDiagramsSecond}>
                                {history.accounts.length > 0 && borderBillionMillionThousand(Math.round(history.accounts.reduce((a, b) => a + Math.abs(b.sum), 0) * 100) / 100)}
          ...
          bottom: 0
                        ...
                        <ButtonHeader onPress={() => {
            setVisible(true);
            activeModalButton.current = true;
          }}>
                            <View>
                                <Image source={require('../../assets/icon/Sorting.png')} style={styles.image} />
                            </View>
                        </ButtonHeader>
                        {dataItems.categories.length > 0 && history.accounts.length > 0 && <ButtonHeader onPress={async () => {
            await getAccount(await getAccounts());
            setVisibleAddHistory(true);
            setactiveModalButtonHistory(true);
          }}>
                                <View>
                                    <Image source={require('../../assets/icon/plus.png')} style={styles.image} />
                                </View>
                            </ButtonHeader>}
                    </ButtonView>
                </CircleContainerBox>
                        ...
                        {dataItems.categories && dataItems.categories.map((item, index) => {
            {
              return <Item key={index}>
                    ...
                    backgroundColor: `${item.color}`
                                            ...
                                            <CardTextSum>
                                                {borderBillionMillionThousand(Math.round(item.value * 100) / 100)}{' '}
                                                руб
                                            </CardTextSum>
                                        </Item>;
            }
