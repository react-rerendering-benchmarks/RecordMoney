...
import { addItem, editItem, getData, setData, borderBillionMillionThousand, replaceSpace } from '../tools/iosys';
...
export default memo(function Account(props: {
  visible: boolean;
  ...
  const onStart = async () => {
    let data: account = await getData({
      fileName: 'Account'
    });
    if (data === null) {
      data = emptyAccount();
  ...
  const onClick = async () => {
    ...
    let dat = {
      id: 0,
      name: text[0].trim(),
      sum: Math.round(+(text[1].trim() != '' ? text[1].trim().replace(',', '.') : '') * 100) / 100
    };
    if (editing.editing) {
  ...
  const del = async (index: number) => {
    Alert.alert('Внимание!', 'При удалении счета будет удалена вся с ним связанная история! Удалить?', [{
      text: 'Да',
      onPress: async () => {
        let newDat: account = JSON.parse(JSON.stringify(state));
  ...
  const transfer = async () => {
    ...
    newState.accounts.filter(item => item.id == +pickerValue)[0].sum += Math.round(+sumEdit.replace(',', '.') * 100) / 100;
    newState.accounts[touchItemIndex].sum = Math.round(newState.accounts[touchItemIndex].sum * 100) / 100;
    newState.accounts.filter(item => item.id == +pickerValue)[0].sum = Math.round(newState.accounts.filter(item => item.id == +pickerValue)[0].sum * 100) / 100;
    setState(newState);
    await setData({
      fileName: 'Account',
      data: newState
  ...
  return <View style={{
    ...
    height: '100%'
            ...
            <ModalWindowOneButton visible={props.visible} setVisible={props.setVisible} windowName={editing.editing ? `Редактирование счёта "${state.accounts[editing.index].name}"` : 'Добавить новый счёт'} functionCancelButton={() => {
      setText(['', '']);
      setEditing({
        editing: false,
        index: 0
      });
    }} functionSaveButton={tryToSave} colorActive='#3EA2FF'>
      ...
      setTouchItemIndex(-1);
        ...
        backgroundColor: 'rgba(0, 0, 0, 0.3)'
            ...
            top: 13
                        ...
                        {accs.length > 1 && <TransferSvg width={25} height={25} style={{
            position: 'absolute',
            right: 13,
            top: 13
          }} onPress={() => {
            if (touchItemIndex !== -1) {
              ...
              setVisible(false);
                ...
                textAlign: 'center'
                                    ...
                                    {touchItemIndex !== -1 ? state.accounts[touchItemIndex].name : ''}
                                </AlertMessage>
                            </AlertInView>
                            <AlertInView>
                                <AlertMessage style={{
                ...
                textAlign: 'center'
                                    ...
                                    {touchItemIndex !== -1 ? borderBillionMillionThousand(state.accounts[touchItemIndex].sum) : ''}{' '}
                                    руб
                                </AlertMessage>
                            </AlertInView>
                        </AlertTextContainer>
              ...
              if (touchItemIndex !== -1) {
                ...
                setVisible(false);
                                ...
                                <AlertButtonText style={{
                color: '#fff'
              }}>Удалить</AlertButtonText>
                            </AlertButton>
                        </AlertButtonCantainer>
                    </ModalInfo>
                </View>
            </Modal>
            <ModalWindowOneButton windowName='Перевод' functionCancelButton={() => {
      setOpenPicker(false);
      setPickerValue('');
      setSumEdit('');
      setModalWindow(false);
      ...
      transfer();
                ...
                <View>
                    <InputPG textName='Сумма' value={sumEdit} setValue={setSumEdit} placeholder={`Не более ${touchItemIndex !== -1 ? state.accounts[touchItemIndex].sum : ''}`} keyboardType='numeric' colorActiveInput='#3EA2FF' />
                    <PickerBlock>
          ...
          }} setOpen={setOpenPicker} setValue={setPickerValue} items={accs.filter(item => {
            return touchItemIndex !== -1 && item.value != accs[touchItemIndex].value;
          })} setItems={setAccs} containerStyle={{
            width: '66%',
            alignSelf: 'flex-end'
