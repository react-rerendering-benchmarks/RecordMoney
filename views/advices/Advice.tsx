...
const ContainerButton = styled.View`
    ...
    height: 200px;
`;
export default memo(function Advice() {
  ...
  const onStart = async () => {
    ...
    let datas: accountHistory = await getData({
      fileName: 'AccountHistory'
    });
    if (data === null) {
  ...
  const GraficCalendar = async () => {
    let data: accountHistory = await getData({
      fileName: 'AccountHistory'
    });
    ...
    const labels = filteredData.map(item => {
      ...
      const dates = date.toLocaleDateString('ru-RU', {
        day: '2-digit',
        year: '2-digit',
        month: '2-digit'
      }).split('/');
  ...
  return <View style={{
    ...
    height: '100%'
      ...
      height={320} yAxisInterval={1} // optional, defaults to 1
      chartConfig={{
        ...
        setVisible2(false);
            ...
            <Modal visible={visible1} animationType='slide' onRequestClose={() => setVisibe1(false)}>
                <LineChart data={{
        labels: HistogrammLabels,
        datasets: [{
          data: Histogrammdata
      ...
      chartConfig={{
        ...
        setVisibe1(false);
            ...
            fontSize: 15
                            ...
                            {history.accHistory.length > 0 && (Math.round(history.accHistory.filter(item => item.type == '2').reduce((a, b) => a + Math.abs(b.sum), 0) * 100) / 100).toString()}
            ...
            fontSize: 15
                            ...
                            {history.accHistory.length > 0 && (Math.round(history.accHistory.filter(item => item.type == '1').reduce((a, b) => a + Math.abs(b.sum), 0) * 100) / 100).toString()}
                            {history.accHistory.length == 0 && '0'} руб.
                        </Text>
                    </TextRow>
                    <TextRow>
            ...
            fontSize: 15
                            ...
                            {debt.debts.length > 0 && (Math.round(debt.debts.filter(item => item.type == '2').reduce((a, b) => a + b.sum, 0) * 100) / 100).toString()}
            ...
            fontSize: 15
                            ...
                            {debt.debts.length > 0 && (Math.round(debt.debts.filter(item => item.type == '1').reduce((a, b) => a + b.sum, 0) * 100) / 100).toString()}
                            {debt.debts.length == 0 && '0'} руб.
                        </Text>
                    </TextRow>
                ...
                <ContainerButton>
                    {visible3 && <ButtonGrafic onPress={GraficCalendar} style={{
          backgroundColor: '#f7c183'
        }}>
            ...
            fontSize: 15
                    ...
                    {visible4 && <ButtonGrafic onPress={GraficDebt} style={{
