...
import { getData, setData, abbrNum, editItem, addItem, delItem, replaceSpace, borderBillionMillionThousand } from '../tools/iosys';
...
function ProgressBar(props: {
  ...
  return <ProgressBarView>
            <ProgressBarCase style={{
      width: `${props.value}%`
    }}></ProgressBarCase>
            <ProgressBarText>{Math.round(props.value)}%</ProgressBarText>
        </ProgressBarView>;
}
export default memo(function PiggyBank() {
  ...
  const search = async () => {
    ...
    let data: piggyBank = await getData({
      fileName: 'PiggyBank'
    });
    if (data === null) {
      data = emptyPiggyBank();
  ...
  const onClick = async () => {
    ...
    if (editing.editing) {
      ...
      if (data.piggyBanks[editSum.index].sum_cur + se === newData.sum_max) {
        ...
        newData.status = true;
        value = se;
        Alert.alert('Поздравляю!', 'Вы закрыли свою цель! Вы можете отслеживать свои закрытые цели в разделе "закрытые". Вы можете удалить их в любое время');
        ...
        Alert.alert('Предупреждение!', 'Вы ввели сумму больше оставшейся, цель закрыта, остатки возвращены на счет');
      } else {
        newData.sum_cur = data.piggyBanks[editSum.index].sum_cur + se;
        value = se;
      }
  ...
  const tryToSave = () => {
    if (replaceSpace(text[0]) === '') {
      ...
      return;
    } else if (!replaceSpace(text[1]).match(/^\d+([\.,]\d{1,2})?$/) || editSum.editSum && !replaceSpace(sumEdit).match(/^\d+([\.,]\d{1,2})?$/) || Number(text[1]) === 0 || editSum.editSum && Number(sumEdit) === 0) {
  ...
  const addMoneyAccount = async (id_accound: number, value: number) => {
    let res = await addMoney(+value * -1, state.piggyBanks[editSum.index].id, id_accound, 'piggyBank');
    return res;
  };
  return <View style={{
    height: '100%',
    backgroundColor: '#fff'
            ...
            <ModalWindowOneButton windowName={editing.editing ? 'Редактирование цели' : 'Добавить новую цель'} functionCancelButton={() => {
      setText(['', '']);
      setPickerValue('');
      setEditing({
        editing: false,
      ...
      tryToSave();
    }} visible={modalWindow} setVisible={setModalWindow} colorActive='#3EA2FF'>
                {!editSum.editSum ? <View>
                        <Input textName='Название' value={text[0].toString()} setItems={setText} index={0} placeholder='Введите название цели' keyboardType='default' colorActiveInput={holdActiveInput ? '#3EA2FF' : '#FF6E6E'} />
                        <Input textName='Сумма' value={text[1].toString()} setItems={setText} index={1} placeholder='Введите сумму' keyboardType='numeric' colorActiveInput={holdActiveInput ? '#3EA2FF' : '#FF6E6E'} />
                    </View> : <View>
                        <InputPG textName='Сумма' value={sumEdit} setValue={setSumEdit} placeholder='Введите сумму' keyboardType='numeric' colorActiveInput={holdActiveInput ? '#3EA2FF' : '#FF6E6E'} />
                        <PickerBlock>
          ...
          }} setOpen={setOpenPicker} setValue={setPickerValue} items={accs} setItems={setAccs} containerStyle={{
            width: '66%',
            alignSelf: 'flex-end'
            ...
            <Header name='Копилка' style='1' functionLeft={useCallback(() => {}, [])} functionRight={useCallback(() => {
      setText(['', '']);
      setModalWindow(true);
      setActiveButtonType(false);
    }, [setText, setModalWindow, setActiveButtonType])} onModalHide={useCallback(async () => {
      await search();
    }, [])} />
            <ButtonTypeView>
                <ButtonType onPress={() => {
        ...
        if (activeButtonType && item.status === true || !activeButtonType && item.status === false) return <CardSwipe key={index} onDelete={useCallback(() => {
          delMode(index);
        }, [])} onEdit={useCallback(() => {
          editMode(index);
          ...
          editModeSum(index);
                                    ...
                                    <CardView>
                                        <CardHeader>
                                            ...
                                            <CardMoney>
                                                ...
                                                {` ${borderBillionMillionThousand(item.sum_max)} руб`}
                                            </CardMoney>
                                        </CardHeader>
                                        <ProgressBar value={+item.sum_cur / +item.sum_max * 100} />
                                        <CardMoreInfo>
                                            До цели осталось
                                            <CardMoreInfo style={{
                fontFamily: 'MainFont-Bold'
                                                ...
                                                {' '}
                                                {abbrNum(Math.round((+item.sum_max - +item.sum_cur) * 100) / 100)}{' '}
                                            </CardMoreInfo>
                                            руб
                                        </CardMoreInfo>
                                    </CardView>
